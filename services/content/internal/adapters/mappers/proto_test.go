...
func TestContentErrorToStatus(t *testing.T) {
	tests := []struct {
		...
		{name: "chat message not found", err: domain.ErrChatMessageNotFound, code: codes.NotFound},
		{name: "permission denied", err: domain.ErrPostForbidden, code: codes.PermissionDenied},
		{name: "chat access forbidden", err: domain.ErrChatAccessForbidden, code: codes.PermissionDenied},
