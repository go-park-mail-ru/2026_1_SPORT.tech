...
func NewLocalMux(ctx context.Context, service contentv1.ContentServiceServer) (http.Handler, error) {
	mux := newMux()
	if err := contentv1.RegisterContentServiceHandlerServer(ctx, mux, service); err != nil {
		return nil, err
	}
...
func NewRemoteMux(ctx context.Context, endpoint string) (http.Handler, error) {
	mux := newMux()
	if err := contentv1.RegisterContentServiceHandlerFromEndpoint(
		ctx,
		mux,
		endpoint,
