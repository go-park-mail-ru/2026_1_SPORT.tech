...
func setDefaults(cfg *Config) {
	if cfg.ServiceName == "" {
		cfg.ServiceName = "profile-service"
	}
	if cfg.Server.Host == "" {
	...
	if cfg.Postgres.Name == "" {
		cfg.Postgres.Name = "sporttech_profile"
	}
	...
	if cfg.Storage.Bucket == "" {
		cfg.Storage.Bucket = "avatars"
	}
	if cfg.Storage.PublicBaseURL == "" {
		cfg.Storage.PublicBaseURL = "http://localhost:8000/avatars"
	}
	if cfg.OpenAPI.FilePath == "" {
		cfg.OpenAPI.FilePath = "grpc/gen/openapiv2/profile/v1/profile.swagger.json"
	}
}
